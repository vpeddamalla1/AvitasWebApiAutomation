...
public class TvImpl extends TestBase implements MediaType {
	...
	public String getMediaTypeUrl() throws Exception{
		return IMDBUtils.getBaseUrl() + Constants.MEDIA_TYPE_TV_URL;
	}

	@Override
	public List<String> getMediaIdsBySearch(String searchTerm) throws Exception {
		// Search for title name
		String searchUrl = IMDBUtils.getSearchUrl(Constants.MEDIA_TYPE_TV);
	...
	public List<String> getAllMyFavorites() throws Exception {
		String getAllFavoritesUrl = IMDBUtils.getAllFavoritesUrl(Constants.MEDIA_TYPE_TV);
		Map<String, String> requestHeaders = new HashMap<>();
		Map<String, String> queryParams = new HashMap<>();
		queryParams.put(Constants.API_KEY, IMDBUtils.getImdbDetails().getApiKey());
	...
	private void updateFavorites(String mediaId, boolean newFlag) throws Exception {
		...
		queryParamsForSession.put(Constants.REQ_BODY_MEDIA_TYPE, Constants.MEDIA_TYPE_TV);
